...
import { ValveModalSPO } from './valve-modal';
...
export const ValveTableSPO = ({ governancevalve, mutateRemove, setSortColumns, sortColumns }: IHydrantTableProps) => {
  const navigate = useNavigate();

  const [removeItem, setRemoveItem] = useState<number | null>(null);

  const handleView = (Id: number) => {
    navigate(`/spo/records/valve/${Id}?edit=false`);
  };

  const handleEdit = (Id: number) => {
    navigate(`/spo/records/valve/${Id}?edit=true`);
  };
