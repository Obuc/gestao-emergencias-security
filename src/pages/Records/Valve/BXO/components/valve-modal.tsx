...
import { ValvePdfBXO } from './valve-pdf';
import TextArea from '@/components/TextArea';
import { Button } from '@/components/Button';
import { Answers } from '@/components/Answers';
import TextField from '@/components/TextField';
import { RespostaValvulas } from '../types/valve.types';
import { useValveModalBXO } from '../hooks/valve-modal.hook';

export const ValveModalBXO = () => {
  const params = useParams();
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const isEdit = searchParams.get('edit') === 'true' ? true : false;

  const { governanceValveItem, setGovernanceValveItem, governaceValveModal, mutateEdit, formik } = useValveModalBXO();
  ...
  const exportToPdf = async () => {
    setGeneratePdf(true);
    const blob = await pdf(<ValvePdfBXO data={governaceValveModal.data} />).toBlob();
