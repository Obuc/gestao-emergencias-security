...
import { InspectionCmiModal, ResponstaInspectionCMI } from '../types/InspectionCmiBXO';

const useInspectionCmiModalBXO = () => {
  ...
  const inspectionCmiModal = useQuery({
    ...
    enabled: params.id !== undefined && pathname === `/records/cmi_inspection/${params.id}` && user_site === 'BXO',
  });
...
export default useInspectionCmiModalBXO;
