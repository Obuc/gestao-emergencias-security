...
import { IInspectionCMIFiltersProps } from '../types/InspectionCmiBXO';
import { sharepointContext } from '../../../../../context/sharepointContext';

const useInspectionCmiBXO = () => {
  ...
  const sessionFiltersActions = sessionStorage.getItem('session_filters_inspection_cmi_bxo');
  ...
  const handleRemoveAllFilters = () => {
    ...
    sessionStorage.removeItem('session_filters_inspection_cmi_bxo');
  };
  ...
  const handleApplyFilters = () => {
    ...
    sessionStorage.setItem('session_filters_inspection_cmi_bxo', JSON.stringify(tempTableFilters));
  };

  const fetch = async ({ pageParam }: { pageParam?: string }) => {
    const orderByQuery = buildOrderByQuery(sortColumns);

    let path = `?$Select=Id,Created,conforme,cmi_idId,site/Title,bombeiro_id/Title&$expand=site,bombeiro_id&$Top=25&${orderByQuery}&$Filter=(site/Title eq 'BXO')`;
  ...
  const inspectionCmi = useInfiniteQuery({
    queryKey: ['inspection_cmi_data_bxo', user_site, tableFilters, sortColumns, year],
    queryFn: fetch,
    getNextPageParam: (lastPage, _) => lastPage.data['odata.nextLink'] ?? undefined,
    staleTime: 1000 * 60,
    enabled: pathname.includes('/records/cmi_inspection') && user_site === 'BXO',
  });
  ...
  const mutateExportExcel = useMutation({
    mutationFn: async () => {
      ...
      if (dataFiltered) {
        ...
        XLSX.writeFile(wb, `Registros BXO - Inspeção CMI.xlsx`);
      }
    },
  });
...
export default useInspectionCmiBXO;
