...
import { AlarmsModalSPO } from './alarms-modal';
...
export const AlarmsTableSPO = ({ alarms, mutateRemove, setSortColumns, sortColumns }: ITableProps) => {
  const navigate = useNavigate();

  const [removeItem, setRemoveItem] = useState<number | null>(null);

  const handleView = (Id: number) => {
    navigate(`/spo/records/fire_alarms/${Id}?edit=false`);
  };

  const handleEdit = (Id: number) => {
    navigate(`/spo/records/fire_alarms/${Id}?edit=true`);
  };
