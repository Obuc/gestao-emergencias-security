...
import { Alarms } from '../types/alarms.types';
import AlarmsModalSPO from './alarms-modal-spo';
...
interface ITableProps {
  alarms: UseInfiniteQueryResult<any, unknown>;
  mutateRemove: UseMutationResult<void, unknown, number, unknown>;
  sortColumns: readonly SortColumn[];
  setSortColumns: React.Dispatch<React.SetStateAction<readonly SortColumn[]>>;
}

const AlarmsTableSPO = ({ alarms, mutateRemove, setSortColumns, sortColumns }: ITableProps) => {
  const navigate = useNavigate();

  const [removeItem, setRemoveItem] = useState<number | null>(null);

  const handleView = (Id: number) => {
    navigate(`/records/fire_alarms/${Id}?edit=false`);
  };

  const handleEdit = (Id: number) => {
    navigate(`/records/fire_alarms/${Id}?edit=true`);
  };
  ...
  const columns: readonly Column<Alarms>[] = [
  ...
  const mappedRows =
    alarms.data?.pages.flatMap(
      (page) =>
        page?.data?.value?.map((item: Alarms) => ({
  ...
  return (
    <>
      <div className="relative">
        ...
        {alarms.isFetchingNextPage && <DataGridLoadMore />}
      </div>
