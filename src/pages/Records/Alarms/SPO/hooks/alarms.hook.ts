...
import { AlarmsFilters } from '../types/alarms.types';
import buildOrderByQuery from '@/utils/buildOrderByQuery';
import { sharepointContext } from '@/context/sharepointContext';

export const useAlarmsSPO = () => {
  ...
  const sessionFiltersActions = sessionStorage.getItem('session_filters_alarms_spo');
  const sessionFiltersActionsJSON: AlarmsFilters = sessionFiltersActions && JSON.parse(sessionFiltersActions);
  ...
  const [tableFilters, setTableFilters] = useState<AlarmsFilters>(initialFiltersValues);
  const [tempTableFilters, setTempTableFilters] = useState<AlarmsFilters>(initialFiltersValues);
  ...
  const handleRemoveAllFilters = () => {
    ...
    sessionStorage.removeItem('session_filters_alarms_spo');
  };
  ...
  const handleApplyFilters = () => {
    ...
    sessionStorage.setItem('session_filters_alarms_spo', JSON.stringify(tempTableFilters));
  };

  const fetch = async ({ pageParam }: { pageParam?: string }) => {
    const orderByQuery = buildOrderByQuery(sortColumns);

    let path = `?$Select=Id,Responsavel1/Title,Local,Created,Sirene,Luminoso&$Expand=Responsavel1&$Top=25&${orderByQuery}&$Filter=(Id ge 0)`;
    ...
    const response = await crudParent.getPaged(pageParam ? { nextUrl: pageParam } : { list: 'Alarmes_de_Incendio', path });
    ...
    const dataWithTransformations = await Promise.all(
      response?.data?.value?.map(async (item: any) => {
        ...
        return {
          ...
          conforme: item.Sirene && item.Luminoso,
        };
      }),
    );
  ...
  const alarms = useInfiniteQuery({
    queryKey: ['alarms_data_spo', user_site, tableFilters, sortColumns, year],
    queryFn: fetch,
    getNextPageParam: (lastPage, _) => lastPage.data['odata.nextLink'] ?? undefined,
    staleTime: 1000 * 60,
    enabled: pathname.includes('/spo/records/fire_alarms') && user_site === 'SPO',
  });

  const mutateRemove = useMutation({
    mutationFn: async (itemId: number) => {
      if (itemId) {
        await crud.deleteItemList('Alarmes_de_Incendio', itemId);
      }
    },
    onSuccess: () => {
  ...
  const fetchAllRecords = async () => {
    const path = `?$Select=Id,Responsavel1/Title,Local,Created,Sirene,Luminoso&$Expand=Responsavel1&$Orderby=Created desc`;

    const response = await crudParent.getListItems('Alarmes_de_Incendio', path);
  ...
  const mutateExportExcel = useMutation({
    mutationFn: async () => {
      ...
      if (dataFiltered) {
        ...
        XLSX.writeFile(wb, `SPO - Registros - Alarmes de Incêndio.xlsx`);
      }
    },
  });
