...
import { EmergencyDoorModalSPO } from './emergency-door-modal';
...
export const EmergencyDoorTableSPO = ({ emergencyDoor, mutateRemove, setSortColumns, sortColumns }: ITableProps) => {
  const navigate = useNavigate();

  const [removeItem, setRemoveItem] = useState<number | null>(null);

  const handleView = (Id: number) => {
    navigate(`/spo/records/emergency_doors/${Id}?edit=false`);
  };

  const handleEdit = (Id: number) => {
    navigate(`/spo/records/emergency_doors/${Id}?edit=true`);
  };
