...
import { ILoadRatioModal, IRespostaLoadRatio } from '../types/loadratio.types';

export const useLoadRatioModalBXO = () => {
  ...
  const loadRatioModal = useQuery({
    queryKey: ['load_ratio_data_modal_bxo', params.id, equipments_value, user_site],
    queryFn: async () => {
      if (params.id) {
    ...
    enabled:
      params.id !== undefined &&
      (pathname.includes('/bxo/records/scania') ||
        pathname.includes('/bxo/records/s10') ||
        pathname.includes('/bxo/records/mercedes') ||
        pathname.includes('/bxo/records/van') ||
        pathname.includes('/bxo/records/iveco') ||
        pathname.includes('/bxo/records/sprinter')) &&
      user_site === 'BXO',
  });
  const mutateEdit = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['load_ratio_data_modal_bxo', params.id, equipments_value, user_site] });
      queryClient.invalidateQueries({ queryKey: ['load_ratio_data_bxo'] });

      const timeoutId = setTimeout(() => {
        setLoadRatioItem(null);
        navigate(`/bxo/records/${equipments_value}`);
