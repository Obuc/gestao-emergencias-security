...
import { TestCMI } from '../types/cmitest.types';
import { TestCmiModalBXO } from './cmitest-modal';
...
interface ITableProps {
  testCmi: UseInfiniteQueryResult<any, unknown>;
  mutateRemove: UseMutationResult<void, unknown, number, unknown>;
  sortColumns: readonly SortColumn[];
  setSortColumns: React.Dispatch<React.SetStateAction<readonly SortColumn[]>>;
}

export const TestCmiCmiTableBXO = ({ testCmi, mutateRemove, setSortColumns, sortColumns }: ITableProps) => {
  const navigate = useNavigate();

  const [removeItem, setRemoveItem] = useState<number | null>(null);

  const handleView = (Id: number) => {
    navigate(`/bxo/records/cmi_test/${Id}?edit=false`);
  };

  const handleEdit = (Id: number) => {
    navigate(`/bxo/records/cmi_test/${Id}?edit=true`);
  };
  ...
  return (
    <>
      <div className="relative">
        ...
        {testCmi.isFetchingNextPage && <DataGridLoadMore />}
      </div>
