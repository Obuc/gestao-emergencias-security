...
import { TestCmiModalSPO } from './cmitest-modal';
...
export const TestCmiTableSPO = ({ cmiTest, mutateRemove, setSortColumns, sortColumns }: ITableProps) => {
  const navigate = useNavigate();

  const [removeItem, setRemoveItem] = useState<number | null>(null);

  const handleView = (Id: number) => {
    navigate(`/spo/records/cmi_test/${Id}?edit=false`);
  };

  const handleEdit = (Id: number) => {
    navigate(`/spo/records/cmi_test/${Id}?edit=true`);
  };
