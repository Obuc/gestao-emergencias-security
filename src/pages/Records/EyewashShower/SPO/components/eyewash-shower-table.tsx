...
import { EyewashShower } from '../types/eyewash-shower.types';
import { EyewashEhowerModalSPO } from './eyewash-shower-modal';
import DataGridLoadMore from '@/components/DataGrid/DataGridLoadMore';

interface ITableProps {
  eyewashShower: UseInfiniteQueryResult<any, unknown>;
  mutateRemove: UseMutationResult<void, unknown, number, unknown>;
  sortColumns: readonly SortColumn[];
  setSortColumns: React.Dispatch<React.SetStateAction<readonly SortColumn[]>>;
}

export const EyewashShowerTableSPO = ({ eyewashShower, mutateRemove, setSortColumns, sortColumns }: ITableProps) => {
  const navigate = useNavigate();

  const [removeItem, setRemoveItem] = useState<number | null>(null);

  const handleView = (Id: number) => {
    navigate(`/spo/records/eyewash_shower/${Id}?edit=false`);
  };

  const handleEdit = (Id: number) => {
    navigate(`/spo/records/eyewash_shower/${Id}?edit=true`);
  };
  ...
  const columns: readonly Column<EyewashShower>[] = [
  ...
  const mappedRows =
    eyewashShower.data?.pages.flatMap(
      (page) =>
        page?.data?.value?.map((item: EyewashShower) => ({
  ...
  return (
    <>
      <div className="relative">
        ...
        {eyewashShower.isFetchingNextPage && <DataGridLoadMore />}
      </div>

      <EyewashEhowerModalSPO />
