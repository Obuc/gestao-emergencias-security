...
import { ExtinguisherModal } from '../types/extinguisher.types';
import { sharepointContext } from '@/context/sharepointContext';

export const useExtinguisherModalSPO = () => {
  ...
  const extinguisherModal = useQuery({
    ...
    queryFn: async () => {
      if (params.id && pathname === `/spo/records/extinguisher/${params.id}` && user_site === 'SPO') {
    ...
    enabled: params.id !== undefined && pathname === `/spo/records/extinguisher/${params.id}` && user_site === 'SPO',
  });
  ...
  const mutateEdit = useMutation({
    ...
    onSuccess: () => {
      ...
      const timeoutId = setTimeout(() => {
        ...
        navigate('/spo/records/extinguisher');
