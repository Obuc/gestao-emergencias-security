...
import { IGeneralChecklistModal } from '../types/generalchecklist.types';

interface IPdfProps {
  data: IGeneralChecklistModal;
}

export const GeneralChecklistPdfBXO = ({ data }: IPdfProps) => {
  return (
    <Document>
      <Page size={[600, 'auto']} wrap style={styles.page}>
        <PDFHeader color="#00354F" title="Gestão de Emergências - Checklist Geral" />

        <View style={styles.container}>
          <PDFContainer.Header color="#00354F" title={`Informações Checklist Geral ${data?.veiculo?.tipo_veiculo}`} />
