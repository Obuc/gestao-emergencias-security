...
import { IGeneralChecklist } from '../types/generalchecklist.types';
import { GeneralChecklistModalBXO } from './generalchecklist-modal';
import DataGridLoadMore from '@/components/DataGrid/DataGridLoadMore';

interface ITableProps {
  generalChecklist: UseInfiniteQueryResult<any, unknown>;
  mutateRemove: UseMutationResult<void, unknown, number, unknown>;
  sortColumns: readonly SortColumn[];
  setSortColumns: React.Dispatch<React.SetStateAction<readonly SortColumn[]>>;
}

export const GeneralChecklistTableBXO = ({ generalChecklist, mutateRemove, setSortColumns, sortColumns }: ITableProps) => {
  const navigate = useNavigate();

  const [removeItem, setRemoveItem] = useState<number | null>(null);

  const handleView = (id: number) => {
    navigate(`/bxo/records/general_checklist/${id}?edit=false`);
  };

  const handleEdit = (id: number) => {
    navigate(`/bxo/records/general_checklist/${id}?edit=true`);
  };
  ...
  const columns: readonly Column<IGeneralChecklist>[] = [
    { key: 'Id', name: '#', resizable: true },
    { key: 'bombeiro/Title', name: 'Responsável', resizable: true, width: 300 },
    { key: 'tipo_veiculo', name: 'Tipo de Veículo', resizable: true },
  ...
  const mappedRows =
    generalChecklist.data?.pages.flatMap(
      (page) =>
        page?.data?.value?.map((item: IGeneralChecklist) => ({
          Id: <div className="pl-4">{item.Id}</div>,
          'bombeiro/Title': item?.bombeiro ? item?.bombeiro : '',
          tipo_veiculo: item?.veiculo?.tipo_veiculo ? item?.veiculo?.tipo_veiculo : '',
  ...
  const handleScroll = async (event: React.UIEvent<HTMLDivElement>) => {
    ...
    generalChecklist.fetchNextPage();
  };
  ...
  return (
    <>
      <div className="relative">
        ...
        {generalChecklist.isFetchingNextPage && <DataGridLoadMore />}
      </div>

      <GeneralChecklistModalBXO />
