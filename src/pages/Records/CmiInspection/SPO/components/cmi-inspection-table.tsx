...
import { InspectionCmiModalSPO } from './cmi-inspection-modal';
...
export const InspectionCmiTableSPO = ({ cmiInspection, mutateRemove, setSortColumns, sortColumns }: ITableProps) => {
  const navigate = useNavigate();

  const [removeItem, setRemoveItem] = useState<number | null>(null);

  const handleView = (Id: number) => {
    navigate(`/spo/records/cmi_inspection/${Id}?edit=false`);
  };

  const handleEdit = (Id: number) => {
    navigate(`/spo/records/cmi_inspection/${Id}?edit=true`);
  };
