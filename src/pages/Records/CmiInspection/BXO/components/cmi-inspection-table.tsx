...
import { InspectionCMI } from '../types/cmi-inspection.types';
import { InspectionCmiModalBXO } from './cmi-inspection-modal';
import DataGridLoadMore from '@/components/DataGrid/DataGridLoadMore';

interface ITableProps {
  inspectionCmi: UseInfiniteQueryResult<any, unknown>;
  mutateRemove: UseMutationResult<void, unknown, number, unknown>;
  sortColumns: readonly SortColumn[];
  setSortColumns: React.Dispatch<React.SetStateAction<readonly SortColumn[]>>;
}

export const InspectionCmiTableBXO = ({ inspectionCmi, mutateRemove, setSortColumns, sortColumns }: ITableProps) => {
  const navigate = useNavigate();

  const [removeItem, setRemoveItem] = useState<number | null>(null);

  const handleView = (Id: number) => {
    navigate(`/bxo/records/cmi_inspection/${Id}?edit=false`);
  };

  const handleEdit = (Id: number) => {
    navigate(`/bxo/records/cmi_inspection/${Id}?edit=true`);
  };
  ...
  const mappedRows =
    inspectionCmi.data?.pages.flatMap(
      (page) =>
        page?.data?.value?.map((item: InspectionCMI) => ({
          Id: <div className="pl-4">{item.Id}</div>,
          'bombeiro_id/Title': item?.bombeiro_id.Title ? item?.bombeiro_id.Title : '',
  ...
  return (
    <>
      <div className="relative">
        ...
        {inspectionCmi.isFetchingNextPage && <DataGridLoadMore />}
      </div>

      <InspectionCmiModalBXO />
