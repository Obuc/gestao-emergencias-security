...
import { Hydrant } from '../types/hydrant.types';
import { HydrantModalSPO } from './hydrant-modal';
...
export const HydrantTableSPO = ({ hydrant, mutateRemove, setSortColumns, sortColumns }: IHydrantTableProps) => {
  const navigate = useNavigate();

  const [removeItem, setRemoveItem] = useState<number | null>(null);

  const handleView = (Id: number) => {
    navigate(`/spo/records/hydrant/${Id}?edit=false`);
  };

  const handleEdit = (Id: number) => {
    navigate(`/spo/records/hydrant/${Id}?edit=true`);
  };
