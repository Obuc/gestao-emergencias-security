...
const OeiTableSPO = ({ oei, mutateRemove, setSortColumns, sortColumns }: ITableProps) => {
  ...
  const columns: readonly Column<Oei>[] = [
    ...
    { key: 'Elevador', name: 'Elevador', resizable: true },
  ...
  const mappedRows =
    oei.data?.pages.flatMap(
      (page) =>
        page?.data?.value?.map((item: Oei) => ({
  ...
  return (
    <>
      <div className="relative">
        ...
        {oei.isFetchingNextPage && <DataGridLoadMore />}
      </div>
