...
import DeaModalSPO from './dea-modal-spo';
...
const DeaTableSPO = ({ dea, mutateRemove, setSortColumns, sortColumns }: ITableProps) => {
  const navigate = useNavigate();

  const [removeItem, setRemoveItem] = useState<number | null>(null);

  const handleView = (Id: number) => {
    navigate(`/records/dea/${Id}?edit=false`);
  };

  const handleEdit = (Id: number) => {
    navigate(`/records/dea/${Id}?edit=true`);
  };
