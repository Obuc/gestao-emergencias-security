...
import { AmbulanceCheck } from '../types/ambulance-check.types';
import AmbulanceCheckModalSPO from './ambulance-check-modal-spo';
import PopoverTables from '../../../../../components/PopoverTables';
import RemoveItem from '../../../../../components/AppModals/RemoveItem';
import DataGridLoadMore from '../../../../../components/DataGrid/DataGridLoadMore';

interface ITableProps {
  ambulanceCheck: UseInfiniteQueryResult<any, unknown>;
  mutateRemove: UseMutationResult<void, unknown, number, unknown>;
  sortColumns: readonly SortColumn[];
  setSortColumns: React.Dispatch<React.SetStateAction<readonly SortColumn[]>>;
}

const AmbulanceCheckTableSPO = ({ ambulanceCheck, mutateRemove, setSortColumns, sortColumns }: ITableProps) => {
  const navigate = useNavigate();

  const [removeItem, setRemoveItem] = useState<number | null>(null);

  const handleView = (Id: number) => {
    navigate(`/records/ambulance_check/${Id}?edit=false`);
  };

  const handleEdit = (Id: number) => {
    navigate(`/records/ambulance_check/${Id}?edit=true`);
  };
  ...
  const columns: readonly Column<AmbulanceCheck>[] = [
  ...
  const mappedRows =
    ambulanceCheck.data?.pages.flatMap(
      (page) =>
        page?.data?.value?.map((item: AmbulanceCheck) => ({
  ...
  return (
    <>
      <div className="relative">
        ...
        {ambulanceCheck.isFetchingNextPage && <DataGridLoadMore />}
      </div>

      <AmbulanceCheckModalSPO />
...
export default AmbulanceCheckTableSPO;
