...
import { AmbulanceCheckModalSPO } from './ambulance-check-modal';
...
export const AmbulanceCheckTableSPO = ({ ambulanceCheck, mutateRemove, setSortColumns, sortColumns }: ITableProps) => {
  const navigate = useNavigate();

  const [removeItem, setRemoveItem] = useState<number | null>(null);

  const handleView = (Id: number) => {
    navigate(`/spo/records/ambulance_check/${Id}?edit=false`);
  };

  const handleEdit = (Id: number) => {
    navigate(`/spo/records/ambulance_check/${Id}?edit=true`);
  };
