...
import { SpillKitQrcodePdf } from './spill-kit-qrcode-pdf';
import { useSpillKitQrCode } from '../hooks/spill-kit-qrcode.hook';
import { SelectAutoComplete } from '@/components/SelectAutocomplete';

interface SpillKitQrcodeModalProps {
  open: boolean | null;
  onOpenChange: () => void;
}

export const SpillKitQrcodeModal = ({ open, onOpenChange }: SpillKitQrcodeModalProps) => {
  ...
  const exportToPdf = async () => {
    ...
    const blob = await pdf(
      <SpillKitQrcodePdf data={selectedItemsSpillKit} pageSize={pageSize.value as StandardPageSize} />,
    ).toBlob();
    saveAs(blob, `QRCode Kit de Derramamento Químico - ${site_value}.pdf`);
  ...
  return (
    <Modal
      ...
      title={`Gerar QRCodes: Kit de Derramamento Químico`}
    >
        ...
        <div className="w-full">
                  ...
                  cmiInspectionQrCodeData.data.map((item) => (
                    <Table.Tr key={item.Id + (item.Title ? item.Title : '0')}>
                      <Table.Td className="pl-8">
                        <Checkbox
                          checked={selectedItemsSpillKit.some((selectedItem) => selectedItem.Id === item.Id)}
          ...
          <div className="flex w-full gap-2 pt-14 justify-end items-center">
            <Button.Root
              ...
              disabled={generatePdf || !selectedItemsSpillKit.length || pageSize === null}
            >
              {generatePdf ? (
                <Button.Spinner />
              ) : (
