...
import { FireAlarmsModal } from './firealarms-modal';
...
interface FireAlarmsTableProps {
  fireAlarmsData: UseInfiniteQueryResult<any, unknown>;
  mutateRemove: UseMutationResult<void, unknown, number, unknown>;
  sortColumns: readonly SortColumn[];
  setSortColumns: React.Dispatch<React.SetStateAction<readonly SortColumn[]>>;
}

export const FireAlarmsTable = ({ fireAlarmsData, mutateRemove, setSortColumns, sortColumns }: FireAlarmsTableProps) => {
  ...
  const columns: readonly Column<FireAlarmsProps>[] = [
  ...
  const mappedRows =
    fireAlarmsData.data?.pages.flatMap(
      (page) =>
        page?.data?.value?.map((item: FireAlarmsProps) => ({
  ...
  return (
    <>
      <div className="relative">
        ...
        {fireAlarmsData.isFetchingNextPage && <DataGridLoadMore />}
      </div>
