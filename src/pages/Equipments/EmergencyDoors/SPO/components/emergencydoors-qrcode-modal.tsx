...
import { EmergencyDoorsQrcodePdf } from './emergencydoors-qrcode-pdf';
import { useEmergencyDoorsQrCode } from '../hooks/emergencydoors-qrcode.hook';

interface EmergencyDoorsQrcodeModalProps {
  open: boolean | null;
  onOpenChange: () => void;
}

export const EmergencyDoorsQrcodeModal = ({ open, onOpenChange }: EmergencyDoorsQrcodeModalProps) => {
  ...
  const exportToPdf = async () => {
    ...
    const blob = await pdf(
      <EmergencyDoorsQrcodePdf data={selectedItemsEmergencyDoors} pageSize={pageSize.value as StandardPageSize} />,
    ).toBlob();
    saveAs(blob, `QRCode Portas de Emergência - ${site_value}.pdf`);
  ...
  return (
    <Modal
      ...
      title={`Gerar QRCodes: Portas de Emergência`}
    >
        ...
        <div className="w-full">
          <div className="max-h-[28.4375rem] w-full overflow-y-auto relative">
            <Table.Root>
              ...
              <Table.Tbody className="overflow-y-scroll">
                {emergencyDoorsQrCodeData.data?.length === 0 && (
                ...
                {emergencyDoorsQrCodeData.isError && (
                  <Table.Tr className="h-14 shadow-xsm text-center font-medium bg-white duration-200">
                    <Table.Td colSpan={5} className="text-center text-primary-font">
                ...
                {emergencyDoorsQrCodeData.isLoading && (
                  <>
                ...
                {emergencyDoorsQrCodeData.data &&
                  emergencyDoorsQrCodeData.data.map((item) => (
                    <Table.Tr key={item.Id + (item.Title ? item.Title : '0')}>
                      <Table.Td className="pl-8">
                        <Checkbox
                          checked={selectedItemsEmergencyDoors.some((selectedItem) => selectedItem.Id === item.Id)}
          ...
          <div className="flex w-full gap-2 pt-14 justify-end items-center">
            <Button.Root
              ...
              disabled={generatePdf || !selectedItemsEmergencyDoors.length || pageSize === null}
            >
              {generatePdf ? (
                <Button.Spinner />
              ) : (
