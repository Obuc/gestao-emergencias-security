...
import { EmergencyDoorsModal } from './emergencydoors-modal';
...
interface EmergencyDoorsTableProps {
  emergencyDoorsData: UseInfiniteQueryResult<any, unknown>;
  mutateRemove: UseMutationResult<void, unknown, number, unknown>;
  sortColumns: readonly SortColumn[];
  setSortColumns: React.Dispatch<React.SetStateAction<readonly SortColumn[]>>;
}

export const EmergencyDoorsTable = ({
  emergencyDoorsData,
  mutateRemove,
  setSortColumns,
  sortColumns,
}: EmergencyDoorsTableProps) => {
  ...
  const columns: readonly Column<EmergencyDoorsProps>[] = [
  ...
  const mappedRows =
    emergencyDoorsData.data?.pages.flatMap(
      (page) =>
        page?.data?.value?.map((item: EmergencyDoorsProps) => ({
  ...
  const handleScroll = async (event: React.UIEvent<HTMLDivElement>) => {
    ...
    emergencyDoorsData.fetchNextPage();
  };
  ...
  return (
    <>
      <div className="relative">
        ...
        {emergencyDoorsData.isFetchingNextPage && <DataGridLoadMore />}
      </div>
