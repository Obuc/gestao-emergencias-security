...
import { EmergencyDoorsFiltersProps, EmergencyDoorsProps } from '../types/emergencydoors.types';

export const useEmergencyDoors = () => {
  ...
  const sessionFiltersActions = sessionStorage.getItem('session_filters_equipments_emergencydoors_spo');
  const sessionFiltersActionsJSON: EmergencyDoorsFiltersProps = sessionFiltersActions && JSON.parse(sessionFiltersActions);
  ...
  const [tableFilters, setTableFilters] = useState<EmergencyDoorsFiltersProps>(initialFiltersValues);
  const [tempTableFilters, setTempTableFilters] = useState<EmergencyDoorsFiltersProps>(initialFiltersValues);
  ...
  const handleRemoveAllFilters = () => {
    ...
    sessionStorage.removeItem('session_filters_equipments_emergencydoors_spo');
  };
  ...
  const handleApplyFilters = () => {
    ...
    sessionStorage.setItem('session_filters_equipments_emergencydoors_spo', JSON.stringify(tempTableFilters));
  };

  const fetchEquipments = async ({ pageParam }: { pageParam?: string }) => {
    const orderByQuery = buildOrderByQuery(sortColumns);

    let path = `?$Select=Id,Modified,Tipo,Predio,Pavimento,Title,Conforme,Excluido&$Top=25&${orderByQuery}&$Filter=(Excluido eq 'false' or Excluido eq null) and (Tipo eq 'Porta')`;
  ...
  const emergencyDoorsData = useInfiniteQuery({
    queryKey: ['equipments_emergencydoors_data_spo', user_site, tableFilters, sortColumns],
  ...
  const fetchAllEquipments = async () => {
    let path = `?$Select=Id,Tipo,Predio,LocEsp,Title,Conforme,Excluido&$Filter=(Excluido eq 'false' or Excluido eq null) and (Tipo eq 'Porta')`;
  ...
  const mutateExportExcel = useMutation({
    mutationFn: async () => {
      ...
      const columns: (keyof EmergencyDoorsProps)[] = ['Id', 'Predio', 'Pavimento', 'Conforme', 'Title'];
      ...
      if (dataFiltered) {
        ...
        XLSX.writeFile(wb, `Equipamentos - Portas de Emergência.xlsx`);
      }
    },
  });
