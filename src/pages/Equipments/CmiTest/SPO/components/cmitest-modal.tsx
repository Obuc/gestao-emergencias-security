...
export const CmiTestModal = () => {
  ...
  const { cmiTestModalData, historyModalData, setYear, year } = useCmiTestModal();
  ...
  const handleOnOpenChange = () => {
    setValveItem(null);
    navigate('/spo/equipments/cmi_test');
  };
            ...
            historyModalData.data?.map((item) => {
              ...
              return (
                <EquipmentCard.Root key={item.Id} variant={cardVariant}>
                  <EquipmentCard.Header title={cardTitle} link={`/spo/records/cmi_test/${item.idRegistro}`} />
