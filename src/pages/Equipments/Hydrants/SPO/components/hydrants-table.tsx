...
const HydrantTable = ({ hydrantData, mutateRemove, setSortColumns, sortColumns }: HydrantTableProps) => {
  ...
  const columns: readonly Column<HydrantsProps>[] = [
    { key: 'Id', name: '#', resizable: true },
    { key: 'numero_hidrante', name: 'Código', resizable: true },
  ...
  const mappedRows =
    hydrantData.data?.pages.flatMap(
      (page) =>
        page?.data?.value?.map((item: HydrantsProps) => ({
          Id: <div className="pl-4">{item.Id}</div>,
          numero_hidrante: item?.numero_hidrante ? item.numero_hidrante : '',
