...
export const useCmiInspection = () => {
  ...
  const sessionFiltersActions = sessionStorage.getItem('session_filters_equipments_cmiinspection_bxo');
  ...
  const handleRemoveAllFilters = () => {
    ...
    sessionStorage.removeItem('session_filters_equipments_cmiinspection_bxo');
  };
  ...
  const handleApplyFilters = () => {
    ...
    sessionStorage.setItem('session_filters_equipments_cmiinspection_bxo', JSON.stringify(tempTableFilters));
  };

  const fetchEquipments = async ({ pageParam }: { pageParam?: string }) => {
    const orderByQuery = buildOrderByQuery(sortColumns);

    let path = `?$Select=Id,cod_qrcode,predio/Title,conforme,Modified,excluido,site/Title,pavimento/Title,tipo_equipamento/Title&$expand=site,pavimento,tipo_equipamento,predio&$Orderby=Modified desc&$Top=25&${orderByQuery}&$Filter=(site/Title eq 'BXO') and (tipo_equipamento/Title eq 'Inspeção CMI') and (excluido eq 'false')`;
  ...
  const cmiInspectionData = useInfiniteQuery({
    queryKey: ['equipments_cmi_inspection_data_bxo', user_site, tableFilters, sortColumns],
    queryFn: fetchEquipments,
    getNextPageParam: (lastPage, _) => lastPage?.data['odata.nextLink'] ?? undefined,
    staleTime: 1000 * 60,
    enabled: user_site === 'BXO' && location.pathname.includes('/bxo/equipments/cmi_inspection'),
  });
  ...
  const fetchAllEquipments = async () => {
    let path = `?$Select=Id,cod_qrcode,conforme,predio/Title,Modified,excluido,site/Title,pavimento/Title,tipo_equipamento/Title&$expand=site,pavimento,tipo_equipamento,predio&$Orderby=Modified desc&$Filter=(site/Title eq 'BXO') and (tipo_equipamento/Title eq 'Inspeção CMI') and (excluido eq 'false')`;
  ...
  const mutateExportExcel = useMutation({
    mutationFn: async () => {
      ...
      const columns: (keyof CmiInspectionProps)[] = ['Id', 'site', 'predio', 'pavimento', 'cod_qrcode', 'conforme'];
