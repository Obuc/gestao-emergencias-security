...
import { CmiInspectionModal } from './cmiInspection-modal';
...
interface CmiInspectionTableProps {
  cmiInspectionData: UseInfiniteQueryResult<any, unknown>;
  mutateRemove: UseMutationResult<void, unknown, number, unknown>;
  sortColumns: readonly SortColumn[];
  setSortColumns: React.Dispatch<React.SetStateAction<readonly SortColumn[]>>;
}

export const CmiInspectionTable = ({
  cmiInspectionData,
  mutateRemove,
  setSortColumns,
  sortColumns,
}: CmiInspectionTableProps) => {
  const navigate = useNavigate();
  const [removeItem, setRemoveItem] = useState<number | null>(null);

  const handleView = (id: number) => {
    navigate(`/bxo/equipments/cmi_inspection/${id}`);
  };
  ...
  const columns: readonly Column<CmiInspectionProps>[] = [
  ...
  const mappedRows =
    cmiInspectionData.data?.pages.flatMap(
      (page) =>
        page?.data?.value?.map((item: CmiInspectionProps) => ({
  ...
  const handleScroll = async (event: React.UIEvent<HTMLDivElement>) => {
    ...
    cmiInspectionData.fetchNextPage();
  };
  ...
  return (
    <>
      <div className="relative">
        ...
        {cmiInspectionData.isFetchingNextPage && <DataGridLoadMore />}
      </div>
