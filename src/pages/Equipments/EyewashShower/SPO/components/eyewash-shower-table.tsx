...
import { EyewashShowerModal } from './eyewash-shower-modal';
...
interface EyewashShowerTableProps {
  eyewashShowerData: UseInfiniteQueryResult<any, unknown>;
  mutateRemove: UseMutationResult<void, unknown, number, unknown>;
  sortColumns: readonly SortColumn[];
  setSortColumns: React.Dispatch<React.SetStateAction<readonly SortColumn[]>>;
}

export const EyewashShowerTable = ({
  eyewashShowerData,
  mutateRemove,
  setSortColumns,
  sortColumns,
}: EyewashShowerTableProps) => {
  const navigate = useNavigate();
  const [removeItem, setRemoveItem] = useState<number | null>(null);

  const handleView = (id: number) => {
    navigate(`/spo/equipments/eyewash_shower/${id}`);
  };
  ...
  const columns: readonly Column<EyewashShowerProps>[] = [
  ...
  const mappedRows =
    eyewashShowerData.data?.pages.flatMap(
      (page) =>
        page?.data?.value?.map((item: EyewashShowerProps) => ({
  ...
  const handleScroll = async (event: React.UIEvent<HTMLDivElement>) => {
    ...
    eyewashShowerData.fetchNextPage();
  };
  ...
  return (
    <>
      <div className="relative">
        ...
        {eyewashShowerData.isFetchingNextPage && <DataGridLoadMore />}
      </div>
