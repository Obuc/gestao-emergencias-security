...
import { EyewashShowerQrcodePdf } from './eyewash-shower-qrcode-pdf';
import { SelectAutoComplete } from '@/components/SelectAutocomplete';
import { useEyewashShowerQrCode } from '../hooks/eyewash-shower-qrcode.hook';

interface EyewashShowerQrcodeModalProps {
  open: boolean | null;
  onOpenChange: () => void;
}

export const EyewashShowerQrcodeModal = ({ open, onOpenChange }: EyewashShowerQrcodeModalProps) => {
  ...
  const exportToPdf = async () => {
    ...
    const blob = await pdf(
      <EyewashShowerQrcodePdf data={selectedItemsEyewashShower} pageSize={pageSize.value as StandardPageSize} />,
    ).toBlob();
    saveAs(blob, `QRCode Chuveiro Lava-Olhos - ${site_value}.pdf`);
  ...
  return (
    <Modal
      ...
      title={`Gerar QRCodes: Chuveiro Lava-Olhos`}
    >
        ...
        <div className="w-full">
                  ...
                  cmiInspectionQrCodeData.data.map((item) => (
                    <Table.Tr key={item.Id + (item.Title ? item.Title : '0')}>
                      <Table.Td className="pl-8">
                        <Checkbox
                          checked={selectedItemsEyewashShower.some((selectedItem) => selectedItem.Id === item.Id)}
          ...
          <div className="flex w-full gap-2 pt-14 justify-end items-center">
            <Button.Root
              ...
              disabled={generatePdf || !selectedItemsEyewashShower.length || pageSize === null}
            >
              {generatePdf ? (
                <Button.Spinner />
              ) : (
