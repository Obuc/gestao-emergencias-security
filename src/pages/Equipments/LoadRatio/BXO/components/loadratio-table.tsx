...
import { LoadRatioModal } from './loadratio-modal';
...
interface LoadRatioTableProps {
  loadRatioData: UseInfiniteQueryResult<any, unknown>;
  mutateRemove: UseMutationResult<void, unknown, number, unknown>;
  sortColumns: readonly SortColumn[];
  setSortColumns: React.Dispatch<React.SetStateAction<readonly SortColumn[]>>;
}

export const LoadRatioTable = ({ loadRatioData, mutateRemove, setSortColumns, sortColumns }: LoadRatioTableProps) => {
  ...
  const columns: readonly Column<LoadRatioProps>[] = [
  ...
  const mappedRows =
    loadRatioData.data?.pages.flatMap(
      (page) =>
        page?.data?.value?.map((item: LoadRatioProps) => ({
  ...
  return (
    <>
      <div className="relative">
        ...
        {loadRatioData.isFetchingNextPage && <DataGridLoadMore />}
      </div>
